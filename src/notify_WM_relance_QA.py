...
def get_migrated_sites() -> list:
    ...
    sites = jira.search_issues('project = WPFEEDBACK AND status = "En attente de feedback"', maxResults=10000)
    return sites
...
def notify_webmasters(key: str, site_name: str, unit: str, webmasters: str, Jahia_url: str) -> None:
    ...
    jinja_template = jinja_env.get_template("20180925_WM_realnce_notification.html")

    # Send the mail
    for webmaster in webmasters.split("|"):
        msgSubject = "[WWP] [{0}] Migration de vos sites de Jahia à Wordpress / Migration of your website from Jahia to Worpress".format(site_name)
    ...
    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
    comment = "{}: Notified '{}' that we still wait for feedback".format(site_name, webmasters)
    jira.add_comment(key, comment)
