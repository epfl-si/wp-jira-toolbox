...
def get_sites_deployed_in_QA_2018() -> list:
    ...
    sites = jira.search_issues('project = WP2018 AND status = "Ventilé en prod18 à notifier"', maxResults=100)
    return sites


def transition_site(key: str, new_status: str, site_name: str, site) -> None:
    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
...
def notify_webmasters(key: str, site_name: str, webmasters: str, wordpress_url: str, QA18_url:str, url_ventilation: str, QA_source: str, url_racine_instance: str, fin_url: str) -> None:
    ...
    jinja_template = jinja_env.get_template("20190606_notify_WM_ventile_prod_2018.py.html")

    # Send the mail
    for webmaster in webmasters.split("|"):
        msgSubject = "[{0}] Votre site a été copié dans l’arborescence www.epfl.ch - Your site has been copied in the www.epfl.ch tree".format(
            site_name)
        msgBody = jinja_template.render(site_name=site_name, webmasters=", ".join(webmasters.split("|")),
                                        ...
                                        url_ventilee=url_ventilation, QA_source=QA_source, url_racine_instance=url_racine_instance, fin_url=fin_url)
        send_message(webmaster, msgSubject, msgBody)

    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
    comment = "{}: Notified '{}' that the site has been copied in an existing instance".format(site_name, webmasters)
    jira.add_comment(key, comment)
...
def ventile_prod():
    ...
    for site in sites:
        ...
        QA_source = site.fields.customfield_10902

        notify_webmasters(site_key, site_name, webmasters, wordpress_url, QA18_url, url_ventilation, QA_source, url_racine_instance, str(url_racine_instance)[-1])
