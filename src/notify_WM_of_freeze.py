...
def get_migrated_sites() -> list:
    ...
    sites = jira.search_issues('project = WPFEEDBACK AND status = "Freeze à communiquer"', maxResults=10000)
    return sites
...
def notify_webmasters(key: str, site_name: str, unit: str, webmasters: str, Jahia_url: str) -> None:
    ...
    jinja_template = jinja_env.get_template("20180516_WM_freeze_jahia_notification.html")

    # Send the mail
    for webmaster in webmasters.split("|"):
        msgSubject = "[WWP] [{0}] Le site '{0}' va être migré sur WordPress / The '{0}' site will be migrated to WordPress".format(site_name)
    ...
    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
    comment = "{}: Notified '{}' that the site should be frozen".format(site_name, webmasters)
    jira.add_comment(key, comment)
...
if __name__ == "__main__":
    ...
    for site in sites:
        ...
        transition_site(site_key, "Début export")
