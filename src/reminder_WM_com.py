...
def get_ready_sites_2018() -> list:
    ...
    sites = jira.search_issues('project = WP2018 AND status = "Communiqu̩ - attente retour WM" AND id= WP2018-594', maxResults=1000)
    return sites
...
def notify_webmasters(key: str, site_name: str, webmasters: str, wordpress_url: str, QA18_url:str, url_ventilation: str, QA_source: str) -> None:
    ...
    jinja_template = jinja_env.get_template("20190716_reminder_WM_com.html")

    # Send the mail
    for webmaster in webmasters.split("|"):
        msgSubject = "[{0}] Rappel: Changement d'identit̩ visuelle / Reminder: New visual identity".format(
            site_name)
    ...
    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
    comment = "{}: Reminder : Notified '{}' to make choice copy ou empty ".format(site_name, webmasters)
