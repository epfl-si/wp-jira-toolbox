...
def get_sites_deployed_in_QA_2018() -> list:
    ...
    sites = jira.search_issues('project = WP2018 AND status = "WP2010" AND cf[10903] = "Assoc"', maxResults=1000)
    return sites
...
def notify_webmasters(key: str, site_name: str, webmasters: str, wordpress_url: str, QA18_url:str, url_ventilation: str, QA_source: str) -> None:
    ...
    jinja_template = jinja_env.get_template("20190529_WM18_assoc_notification_choice.html")

    # Send the mail
    for webmaster in webmasters.split("|"):
        msgSubject = "[{0}] changement identité visuelle / New visual identity".format(
            site_name)
    ...
    jira = JIRA(settings.JIRA_URL, basic_auth=(settings.JIRA_USERNAME, settings.JIRA_PASSWORD))
    comment = "{}: Notified '{}' assoc to make choice copy ou empty ".format(site_name, webmasters)
    jira.add_comment(key, comment)
